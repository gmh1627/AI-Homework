...
#@dataclass用于简化数据类（data classes）的创建，会自动为类添加特殊方法，如 __init__ 和 __repr__。
#frozen=True 参数表示创建的数据类是不可变的
class GeoLocation:
...
class CityMap:
    ...
    """
    def __init__(self) -> None:#-> None 表示这个函数没有返回值
        ...
        self.tags: Dict[str, List[str]] = defaultdict(list)#defaultdict(list)表示如果尝试访问字典中不存在的键，将返回一个空列表，而不是引发KeyError
