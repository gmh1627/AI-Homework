...
class Gobang(UtilGobang):
    ...
    def sample_noise(self) -> Union[Tuple[int, int, int], None]:
        ...
        if self.action_space:
            ...
            choosen_position = random.choice(self.action_space)
            x = choosen_position[0]
            y = choosen_position[1]
            self.action_space.remove(choosen_position)
            # END_YOUR_CODE
            return 2, x, y
        else:
            return None
    ...
    def q_learning_update(self, s0_: np.array, action: Tuple[int, int, int], s1_: np.array, reward: float,
        ...
        if s0 not in self.Q:
            self.Q[s0] = {}
        if tuple(action) not in self.Q[s0]:
            self.Q[s0][tuple(action)] = 0
        if s1 not in self.Q:
            self.Q[s1] = {}
        opt_v = max(self.Q[s1].values()) if self.Q[s1] else 0
        self.Q[s0][tuple(action)] = self.Q[s0][tuple(action)] + alpha * (reward + self.gamma * opt_v - self.Q[s0][tuple(action)])
        # END_YOUR_CODE
