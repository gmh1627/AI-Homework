...
class Actor(nn.Module):
    ...
    def __init__(self, board_size: int, lr=1e-4):
        ...
        heads=4
        mlp_dim=128
        assert board_size % patch_size == 0, 'Image dimensions must be divisible by the patch size.'
    ...
    def forward(self, x: np.ndarray):
        ...
        # BEGIN YOUR CODE
        mask = (output.flatten(start_dim=2) == 0).float()
        ...
        output = rearrange(output, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=patch_size, p2=patch_size)
        # print(output.shape)
        # output = self.linear(output)
        output = self.to_patch_embedding(output)
        b, n, _ = output.shape

        cls_tokens = self.cls_token.expand(b, -1, -1).to(device)  # Move to device
        output = torch.cat((cls_tokens, output), dim=1).to(device)  # Move to device
        output += self.pos_embedding[:, :(n + 1)].to(device)  # Move to device
        ...
        mask_ = output != 0
        output_ = torch.where(mask_, output, torch.tensor(-1e10).to(device))
        softmax_output = nn.functional.softmax(output_, dim=1)
        softmax_output_ = torch.where(mask_, softmax_output, output)
        output = softmax_output_
        # END YOUR CODE
        return output
...
class Critic(nn.Module):
    ...
    def forward(self, x: np.ndarray, action: np.ndarray):
        ...
        b, c, h, w = x.shape
        output = rearrange(x, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=patch_size, p2=patch_size)
        # print(output.shape)
        # output = self.linear(output)
        output = self.to_patch_embedding(output)
        b, n, _ = output.shape
        ...
        cls_token = output[:, 0]
        output = self.mlp_head(cls_token)
        output = output.gather(1, indices.unsqueeze(1)).squeeze(1)
        # END YOUR CODE
